...
namespace Microsoft.AspNetCore.Mvc.Internal
{
    public abstract class ResourceInvoker
    {
                ...
                case State.AuthorizationShortCircuit:
                    {
                        ...
                        return InvokeResultFilterPipeline(ResultExecutionScope.Authorization);
                    }
                ...
                case State.ResourceShortCircuit:
                    {
                        ...
                        var task = InvokeResultFilterPipeline(ResultExecutionScope.Resource);
                        if (task.Status != TaskStatus.RanToCompletion)
                        {
                ...
                case State.ExceptionHandled:
                    {
                        ...
                        var task = InvokeResultFilterPipeline(ResultExecutionScope.Exception);
                        if (task.Status != TaskStatus.RanToCompletion)
                        {
                ...
                case State.ExceptionEnd:
                    {
                        ...
                        var task = InvokeResultFilterPipeline(ResultExecutionScope.Action);
                        if (task.Status != TaskStatus.RanToCompletion)
                        {
                            next = State.ResourceInsideEnd;
                            return task;
        ...
        private async Task InvokeResultFilterPipeline(ResultExecutionScope resultExecutionScope)
        {
            var resultNext = State.ResultBegin;
            var resultScope = Scope.Invoker;
            var resultState = (object)null;
            var resultIsCompleted = false;

            while (!resultIsCompleted)
            {
                if (resultExecutionScope == ResultExecutionScope.Action)
                {
                    await ResultNext<IResultFilter, IAsyncResultFilter>(ref resultNext, ref resultScope, ref resultState, ref resultIsCompleted);
                }
                else
                {
                    Debug.Assert(
                        resultExecutionScope == ResultExecutionScope.Authorization ||
                        resultExecutionScope == ResultExecutionScope.Exception ||
                        resultExecutionScope == ResultExecutionScope.Resource);
                    await ResultNext<IAlwaysRunResultFilter, IAsyncAlwaysRunResultFilter>(ref resultNext, ref resultScope, ref resultState, ref resultIsCompleted);
                }
            }
        }
        ...
        private enum ResultExecutionScope
        {
            Action,
            Authorization,
            Resource,
